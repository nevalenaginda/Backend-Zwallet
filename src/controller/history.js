...
module.exports = {
  ...
  listHistoryAdmin: async (req, res) => {
    try {
      const param = req.query.param ? req.query.param : "created_at";
      const sort = req.query.sort ? req.query.sort : "ASC";
      const limit = req.query.limit ? req.query.limit : 6;
      const page = req.query.page ? req.query.page : 1;
      const offset = page === 1 ? 0 : (page - 1) * limit;
      const responseTotal = await mTotalAdmin(); // count total page
  ...
  listNotifications: async (req, res) => {
    try {
      ...
      const param = req.query.param ? req.query.param : "created_at";
      const sort = req.query.sort ? req.query.sort : "ASC";
      const limit = req.query.limit ? req.query.limit : 60;
      const page = req.query.page ? req.query.page : 1;
      const offset = page === 1 ? 0 : (page - 1) * limit;
      const responseTotal = await mTotalNotifications(id); // count total page
      ...
      mListNotifications(id, param, sort, offset, limit)
        .then((response) => {
          ...
          if (response.length > 0) {
            success(res, data, pagination, "Get all notifications success");
          } else {
            const pagination = {
              page: page,
              limit: limit,
      ...
      mUpdateHistory(data, id)
        .then((response) => {
          mDetailHistoryFix(id)
            .then((response) => {
              if (response.length > 0) {
                success(res, response[0], {}, "Get detail history success");
              } else {
                success(res, null, {}, "Get detail history success");
              }
            })
            .catch(() => {
              failed(res, "Internal server error", []);
